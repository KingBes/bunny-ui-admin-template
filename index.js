...
document.body.addEventListener("htmx:load", function () {
    if (!is_menu) {
        ...
        if (menu) {
            const id = localStorage.getItem("default_menu_key")
            const item = htmx.find(menu, "li>a")
            item.click()
            if (id !== null && id != 0) {
                htmx.find(`[hx-ext="bny-tab-add"][bny-id="${id}"]`).click()
                localStorage.setItem("default_menu_key", id)
            }
            is_menu = true
        }
    }
...
document.body.addEventListener("click", function (event) {
    ...
    if (target.getAttribute("hx-ext") === "bny-tab-add") {
        const id = target.getAttribute("bny-id")
        localStorage.setItem("default_menu_key", id)
    }
})
